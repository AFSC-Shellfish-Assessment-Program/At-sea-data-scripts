...
#    - Must have a Network/VPN connection to access most up-to-date haul info
#
...
# Update real-time haul data ---------------------------------------------------
#**Only Shannon and Emily have the Oracle credentials to do this step!*

  # Connect to Oracle
    channel <- RODBC::odbcConnect(dsn = "AFSC", 
                                  uid = getPass::getPass(msg = "USERNAME: "),
                                  pwd = getPass::getPass(),
                                  believeNRows = FALSE)
  
  # Read in standard station metadata for grid cell midpoint coordinates
    stations <- read.csv("Y:/KOD_Survey/EBS Shelf/Data_Processing/Data/lookup_tables/station_lookup.csv", header = TRUE) %>%
                select(-STATION_TYPE)
  
  # Pull haul info from RACE_DATA tables
    haul <- RODBC::sqlQuery(channel = channel, 
                            query = paste0("SELECT *
                                            FROM RACE_DATA.SURVEYS ss, RACE_DATA.CRUISES cc, RACE_DATA.EDIT_HAULS hh
                                            WHERE ss.SURVEY_ID = cc.SURVEY_ID
                                            AND cc.CRUISE_ID = hh.CRUISE_ID
                                            AND SURVEY_DEFINITION_ID IN (98, 143) -- 98 EBS, 143 NBS
                                            AND YEAR = ", current_year, ";")) %>%
            select(SURVEY_ID, YEAR, CRUISE, VESSEL_ID, START_DATE, END_DATE, # CRUISE_LEG_ID, SURVEY_DEFINITION_ID, 
                   HAUL, STATION, HAUL_TYPE, PERFORMANCE, GEAR, ACCESSORIES)
  ...
  # Save table 
    write.csv(haul_table, "Y:/KOD_Survey/EBS Shelf/Data_Processing/Data/haul_atsea.csv", row.names = FALSE)
...
# Additional data wrangling to inform resampling decision ----------------------
#**NOT needed for table output*

  # Summarize threshold by shell condition - proxy for proportion of females that 
  # have molted, but not yet mated (SC 0 -2)
    data %>%
      filter(SPECIES_CODE == 69322,
             SEX == 2,
             CLUTCH_SIZE != 0) %>% 
