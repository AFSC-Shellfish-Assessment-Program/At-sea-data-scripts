...
spec<- list.files("C:/Users/jon.richar/Work/GitRepos/At-sea-data-scripts/Data/DataFromBoats/",pattern = "\\.accdb$") 
...
for(i in 1:length(spec)){ # start i loop (each file)
  path <- paste0("C:/Users/jon.richar/Work/GitRepos/At-sea-data-scripts/Data/DataFromBoats/", spec[i], sep="")
  x<-odbcConnectAccess2007(path)
  out<-sqlFetch(x, "tblCRAB_SPECIMEN")
  out<-data.frame(out)
  dat<-rbind(dat,out)
...
write.csv(stations, "C:/Users/jon.richar/Work/GitRepos/At-sea-data-scripts/Output/Resample_station_list.csv")
...
gt_two_column_layout(listed_tables, output = "save", 
                     filename = paste0(run_date, "_THRESHOLD.png"),
                      path = "C:/Users/jon.richar/Work/GitRepos/At-sea-data-scripts/Output") 
